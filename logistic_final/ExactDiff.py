...
class ExactDiff(object):
    ...
    def __init__(self, problem, algo_type, graph_type, num_agent, logMaxIter, data_dir, save_dir, log_scale=False, batch=5):
        ...
        self.phi = np.copy(self.theta) #这几个算法对phi的初始化非常敏感
    ...
    def save(self, rep):        
        if self.algo_type == 'homo':
            np.save(self.save_dir + f"rep{rep}-ex-homo-{self.graph}.npy", self.metric)
        elif self.algo_type == 'hete':
            np.save(self.save_dir + f"rep{rep}-ex-hete-{self.graph}.npy", self.metric)
        else:
    ...
    def fit(self, rep):
        ...
        for t in range(self.maxIter):
            ...
            phi_new = self.theta - self.stepsize(t) * new_grd
            temp = phi_new + self.theta - self.phi
            self.theta = self.communicator.agg(temp, op="weighted_avg")
            self.phi = np.copy(phi_new)
            ...
            if self.rank == 0 and t % 100 == 0:
                print(f'Exact Diffusion-{self.algo_type}: Iter Percentage {t/self.maxIter:.0%}, MSE: {self.metric["mse"][-1]:.3e}, Worst MSE: {self.metric["wmse"][-1]:.3e}')
...
if __name__ == "__main__":
    ...
    for rep in range(num_trails):
        ...
        hete_gt = ExactDiff(problem, algo_type='hete', graph_type=graph, num_agent=num_agent, logMaxIter=5, save_dir=dir, batch=4, data_dir = 'data/', log_scale=True)
        hete_gt.fit(rep=rep)

        homo_gt = ExactDiff(problem, algo_type='homo', graph_type=graph, num_agent=num_agent, logMaxIter=5, save_dir=dir, batch=4, data_dir = 'data/', log_scale=True)
        homo_gt.fit(rep=rep)
