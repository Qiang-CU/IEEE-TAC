...
csgd = CSGD(problem, num_agent, batch, logMaxIter, save_dir, lg_flag=False)
csgd.fit()

for rep in range(num_trails):
    homo_dsgd = DSGD(problem, algo_type='homo', graph_type=graph, num_agent=num_agent, logMaxIter=logMaxIter, save_dir=save_dir, data_dir=data_dir, batch=batch, lg_flag=False)
    homo_dsgd.fit(rep=rep)

    hete_dsgd = DSGD(problem, algo_type='hete', graph_type=graph, num_agent=num_agent, logMaxIter=logMaxIter, save_dir=save_dir, data_dir=data_dir, batch=batch, lg_flag=False)
