...
num_agent = 20 #int(sys.argv[1]) # Note: input's type is str, you need to change the type
logMaxIter = 4
batch = 1
...
for rep in range(num_trails):
    homo_dsgd = DSGD(problem, algo_type='homo', graph_type=graph, num_agent=num_agent, logMaxIter=logMaxIter, save_dir=save_dir, data_dir=mixmat_dir, batch=batch, lg_flag=False, a0=10, a1 = 500)
    homo_dsgd.fit(rep=rep)

    hete_dsgd = DSGD(problem, algo_type='hete', graph_type=graph, num_agent=num_agent, logMaxIter=logMaxIter, save_dir=save_dir, data_dir=mixmat_dir, batch=batch, lg_flag=False, a0=5, a1=5000)
