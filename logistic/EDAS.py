...
class EDAS(object):
    ...
    def __init__(self, problem, algo_type, graph_type, num_agent, logMaxIter, data_dir, save_dir, log_scale=False, batch=5):
        ...
        self.last_grd = np.zeros(self.dim + 1,)
    ...
    def save(self, rep):        
        if self.algo_type == 'homo':
            np.save(self.save_dir + f"rep{rep}-edas-homo-{self.graph}.npy", self.metric)
        elif self.algo_type == 'hete':
            np.save(self.save_dir + f"rep{rep}-edas-hete-{self.graph}.npy", self.metric)
        else:
    ...
    def fit(self, rep):
        ...
        for t in range(self.maxIter):
            ...
            X, Y = self.sample()
            grd = self.problem.stoc_grad(X, Y, self.theta)

            if t == 0:
                self.inter = self.theta - self.stepsize(t) * grd
            else:
                self.inter = 2*self.theta - self.last_theta - self.stepsize(t) * grd + self.stepsize(t-1) * self.last_grd
            
            self.last_grd = np.copy(grd)
            self.last_theta = np.copy(self.theta)
            self.communication()
            ...
            if self.rank == 0 and t % 100 == 0:
                print(f'EDAS-{self.algo_type}: Iter Percentage {t/self.maxIter:.0%}, MSE: {self.metric["mse"][-1]:.3e}, Worst MSE: {self.metric["wmse"][-1]:.3e}')
...
if __name__ == "__main__":
    ...
    for rep in range(num_trails):
        ...
        hete = EDAS(problem, algo_type='hete', graph_type=graph, num_agent=num_agent, logMaxIter=5, save_dir=dir, batch=4, data_dir = 'data/', log_scale=True)
        hete.fit(rep=rep)

        homo = EDAS(problem, algo_type='homo', graph_type=graph, num_agent=num_agent, logMaxIter=5, save_dir=dir, batch=4, data_dir = 'data/', log_scale=True)
        homo.fit(rep=rep)
